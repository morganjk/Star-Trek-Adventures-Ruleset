...
function addInfoDB(nodeChar, sClass, sRecord)
	...
	if sClass == "species" then
		addSpeciesRef(nodeChar, sClass, sRecord);
	elseif sClass == "environment" then
		addEnvironmentRef(nodeChar, sClass, sRecord);
	elseif sClass == "upbringing" then
		addUpbringingRef(nodeChar, sClass, sRecord);
...
function addSpeciesRef(nodeChar, sClass, sRecord)
	...
	-- Notify
	outputUserMessage("char_abilities_message_speciesadd", DB.getValue(nodeSource, "name", ""), DB.getValue(nodeChar, "name", ""));

	-- Add the name and link to the main character sheet
	DB.setValue(nodeChar, "details.species", "string", DB.getValue(nodeSource, "name", ""));
	DB.setValue(nodeChar, "specieslink", "windowreference", sClass, nodeSource.getNodeName());
end

function addEnvironmentRef(nodeChar, sClass, sRecord)
	local nodeSource = resolveRefNode(sRecord);
	if not nodeSource then
		return;
	end
	...
	-- Notify
	outputUserMessage("char_details_message_environmentadd", DB.getValue(nodeSource, "name", ""), DB.getValue(nodeChar, "name", ""));

	-- Add the name and link to the main character sheet
	DB.setValue(nodeChar, "details.environment", "string", DB.getValue(nodeSource, "name", ""));
	DB.setValue(nodeChar, "environmentlink", "windowreference", sClass, nodeSource.getNodeName());
end

function addUpbringingRef(nodeChar, sClass, sRecord)
	local nodeSource = resolveRefNode(sRecord);
	if not nodeSource then
		return;
	end
	...
	-- Notify
	outputUserMessage("char_details_message_upbringingadd", DB.getValue(nodeSource, "name", ""), DB.getValue(nodeChar, "name", ""));

	-- Add the name and link to the main character sheet
	DB.setValue(nodeChar, "details.upbringing", "string", DB.getValue(nodeSource, "name", ""));
	DB.setValue(nodeChar, "upbringinglink", "windowreference", sClass, nodeSource.getNodeName());
end
